/**	MySQL / MariaDB Quirks
	...
	ANSI Mode
		...
		These samples all presume ANSI mode.
	
		If you can you should run this at the beginning:
	
			SET GLOBAL sql_mode = 'ANSI';
	
		Failing that, you should run:
	
			SET SESSION sql_mode = 'ANSI';
	
		You can test whether ANSI mode has been set with:

			SELECT @@GLOBAL.sql_mode;
			SELECT @@SESSION.sql_mode;

		If you can’t use ANSI mode, you can’t concatenate
	...
	Other Quirks
		...
		Dates
			...
			You can use alternative literals with str_to_date():
				SELECT * FROM customers
				WHERE dob=str_to_date('09-NOV-1989', '%d-%M-%Y');
	
		SELECT *
			You cannot include specific columns after SELECT *:
				SELECT *,column		--	ok
				SELECT column,*		--	fails
		...
		Regular Expressions
			You can use regular expressions with the RLIKE keyword:
				'/ … /' RLIKE
	
		Case Sensitivity
			By default, strings are case insensitive.
			However, Table Names may be case sensitive, depending on
			the operating system.

		Casting to a String
			In MySQL only, you can’t cast to VARCHAR(…). You must
			cast to CHAR(…), which is not, in this case, fixed length (‽).

			MariaDB allows both CHAR(…) and VARCHAR(…).

		Casting to an Integer
			In MySQL only, you can’t cast to INT. You must
			cast to SIGNED/UNSIGNED.

			MariaDB allows both INT and SIGNED/UNSIGNED.
...
/*	02.07	Drop Old Customer Address Columns
	================================================
	================================================ */
...
/*	02.08b	Update via View
	================================================
	================================================ */

	UPDATE customerdetails SET townid=0 WHERE id=42;	--	use original townid value
...
/*	2.18: Altering a Column in SQLite
	================================================
	================================================ */
...
/*	3.11: Joining with Summarised Genres
	...
	WITH cte AS (
		SELECT bg.bookid, group_concat(g.genre separator ', ') AS genres
...
/*	3.14: Combining Multiple Authors
	...
	WITH cte AS (
		SELECT
			ba.bookid,
			group_concat(a.givenname||' '||a.familyname separator ' & ') AS authors
...
/*	3.16: Getting the new ID
	...
	--	Select both statements and run:
		INSERT INTO authors(givenname, othernames, familyname,
			born, died, gender, home)
		VALUES('Agatha', 'Mary Clarissa', 'Christie',
			'1890-09-15', '1976-01-12', 'f',
...
/*	4.18: Casting from Strings
	================================================
	MariaDB and MySQL differ in casting integers
		MariaDB/MySQL:	cast(numberstring as int/signed/unsigned)
		MySQL Only:		cast(numberstring as signed/unsigned)
	================================================ */
	...
	--	Integers
		...
		SELECT * FROM sorting
		--	ORDER BY cast(numberstring as int); --	MariaDB Only
		ORDER BY cast(numberstring as signed);	--	MariaDB / MySQL
...
/*	4.23: Casting to narrow number type
	...
	SELECT
		cast(234.567 AS unsigned) AS castint,
...
/*	4.25: String Literal
	================================================
	MariaDB / MySQL:
		"double" quotes alternative in Traditional Mode
	================================================ */
...
/*	4.28: String Functions
	...
	--	Position
		SELECT *, instr(title, ' ') AS space FROM books;
...
/*	4.29: Substrings
	...
	--	Right
		...
		substr('abcdefghijklmnop',-4) AS rstring_too
	FROM vars;
...
/*	5.05: Mixing Aggregates and Non-Aggregates
	================================================
	MariaDB / MySQL:
		You can include non-aggregates, but you’ll get
		an indeterminite result.
		This is useful if you know there’s only one.
	================================================ */
...
/*	5.11: Grouping by Month Name
	================================================
	SQLite: Month name is not available
	================================================ */

	SELECT
		month(dob) AS monthnumber,
		monthname(dob) AS monthname,
		count(*) AS howmany
	FROM customerdetails
...
/*	5.16: Ordering by Strings
	...
	FROM statuses
	GROUP BY status
	ORDER BY INSTR(status, 'Shipped,Shipped Late,Current,Due,Overdue')
	;
...
/*	5.24: GROUPING SETS (PostgreSQL, MSSQL, and Oracle)
	================================================
	================================================ */

/*	5.25: CUBE (PostgreSQL, MSSQL, and Oracle)
	================================================
	================================================ */
...
/*	5.26: ROLLUP
	...
	FROM salesdata
	GROUP BY state, customerid, ordered WITH ROLLUP;
...
/*	5.28: Renaming Totals
	...
	MariaDB / MySQL:
		cast(… as char(…))
	MariaDB Only can also:
		cast(… as varchar(…))
	================================================ */

	--	MySQL (MariaDB doesn't support grouping())
		SELECT
			coalesce(state, 'National Total') AS state,
			coalesce(cast(customerid as char(12)), state||' Total') AS customerid,
			coalesce(ordered, 'Total for '||cast(customerid as char(12))) AS ordered,
			count(*), sum(total)
		FROM salesdata
		GROUP BY state, customerid, ordered WITH ROLLUP
		ORDER BY
...
/*	5.32: Calculating the Median: Aggregate Function
	================================================
	================================================ */

	--	Repeated over Table
		SELECT percentile_cont(0.5) WITHIN GROUP (ORDER BY height) OVER()
		FROM customers
		WHERE height IS NOT NULL;

	--	Single Value
		SELECT DISTINCT percentile_cont(0.5) WITHIN GROUP (ORDER BY height) OVER()
		FROM customers
		WHERE height IS NOT NULL;
...
/*	6.07: Computed Columns
	...
	ALTER TABLE sales
	ADD ordered_date date GENERATED ALWAYS AS (cast(ordered as date)) STORED;
...
/*	7.17: Lateral Joins / Cross Apply: Unsuccessful Use Cases
	================================================
	================================================ */
...
/*	7.18: Using Lateral Join / Cross Apply: Calculated Aliases
	================================================
	MariaDB only:
		Not Availabe
	================================================ */
...
/*	7.19: Using Lateral Join / Cross Apply: Fetching Multiple Columns
	================================================
	MariaDB only:
		Not Availabe
	================================================ */
...
/*	7.20: Using Lateral Join / Cross Apply: Using an Aggregate Query
	================================================
	MariaDB only:
		Not Availabe
	================================================ */
...
/*	8.01: Simple Aggregate Window
	...
	--	This gives an indeterminite result for id:
		SELECT
			id, givenname, familyname,
			count(*)
		FROM customerdetails;
...
/*	8.03: Using Aggregate OVER()
			Compare total to Average
	================================================
	================================================ */
...
/*	8.04: Sales by Week Day
	...
	--	Embellish Proportion
		...
		SELECT
			weekday_number, total,
			format(100*total/sum(total) OVER(), 2) || '%' AS proportion
		FROM summary
		ORDER BY weekday_number;
...
/*	8.07: Creating a daily_sales View
	...
	SELECT
		ordered_date,
		date_format(ordered_date, '%Y-%m') AS ordered_month,
...
/*	8.10: ORDER BY and PARTITION BY Sampler
	...
	SELECT
		...
		--	MSSQL
			sum(daily_total) OVER(ORDER BY cast(ordered_month as char(7))) AS running_month_total,
		sum(daily_total) OVER(
			PARTITION BY ordered_month
			ORDER BY ordered_date ROWS UNBOUNDED PRECEDING
		) AS month_running_total
	...
	--	MySQL (not MariaDB)
		WITH dates(dob, today) AS (
			VALUES
				...
				row('1940-10-09', '2022-12-31'),
				row('1940-07-07', '2023-07-07'),
				row('1943-02-25', '2023-02-25'),
				row('1942-06-18', '2023-06-18'),
				row('1940-10-09', '2023-10-09')
		)
		SELECT
			dob, today,
			timestampdiff(year, dob, current_timestamp) AS age
...
/*	9.15: Using a Table Literal for Sorting
	...
	--	MariaDB
		WITH
			data AS (
				SELECT date_format(ordered, '%W') AS weekday, total
				FROM sales
			),
			summary AS (
...
/*	9.17: Using a Recursive CTE to Split a String
	...
	--	MariaDB (not MySQL)
		WITH RECURSIVE
			...
			split(fruit, rest) AS (
				SELECT
					substring(fruits, 1, position(',' in fruits)-1),
	...
	--	MySQL (Not MariaDB)
		WITH RECURSIVE
			cte(fruits) AS (
				VALUES row('Apple,Banana,Cherry,Date,Elderberry,Fig')
			),
			split(fruit, rest) AS (
				SELECT
					substring(fruits, 1, position(',' in fruits)-1),
...
/*	9.18: Splitting More Complex Data
	...
	--	MySQL (Not MariaDB)
		WITH RECURSIVE
			cte(name, items) AS (
				VALUES
					row('colours', 'Red,Orange,Yellow,Green,Blue,Indigo,Violet'),
					row('elements', 'Hydrogen,Helium,Lithium,Beryllium,Boron,Carbon'),
					row('numbers', 'One,Two,Three,Four,Five,Six,Seven,Eight,Nine')
			),
			split(name, item, rest) AS (
				SELECT
					name,
...
/*	10.01: Archive Trigger - MariaDB / MySQL
	================================================
...
/*	10.01b	Archive Trigger - Test
	...
	SELECT * FROM sales WHERE ordered IS NULL;
	DELETE FROM sales WHERE ordered IS NULL;
...
/*	10.05: Manually Pivoting Data: Summarise
	================================================
	================================================ */
...
/*	10.10: Using Variables in MariaDB / MySQL
	================================================
	MariaDB:
		BEGIN NOT ATOMIC
			…
	...
	DELIMITER $$		--	MySQL: Omit
	BEGIN NOT ATOMIC	--	MySQL: Omit
		SET @cid = 42;
